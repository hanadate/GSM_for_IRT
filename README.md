# GSM_for_IRT
use mainly 8 scripts as follows:
- dif_power_easy.R, dif_power_middle.R, dif_power_difficult.R
-- DIF detection using glmer under fixed sample (for simulation 1).
- dif_sequential_multi_easy.R, dif_sequential_multi_middle.R, dif_sequential_multi_difficult.R
-- DIF detection using glmer under sequential design (for simulation 2).
- empirical_dif.R
-- Application of provided group sequential method for empirical data.
- bias_PQL.R
-- Investigation of estimation bias using PQL
