...
for (k in 1:nrow(params)) {
  ...
  write_csv(zvalues, paste0("zvalues_easy/zvalues_easy_",k,".csv"))
  gc()
}
(now()-t)
stopCluster(cl)
folder_path <- "zvalues_easy"
file_list <- list.files(folder_path, pattern = "\\.csv$", full.names = TRUE, recursive = TRUE)
res <- file_list %>%
  lapply(read_csv) %>%
  bind_rows()
res
write_csv(res, "zvalues_easy_all.csv")
res <- read_csv("zvalues_easy_all.csv")
...
pocock_stage %>%
  ...
  stargazer(., digits=2)
pocock_stage %>% 
  mutate(position="easy",
         boundary="pocock",
         .before=1) %>% 
  write_csv("dif_sequential_multi_easy_pocock.csv")
# pocock_stage %>% View
...
of_stage %>% 
  mutate(position="easy",
         boundary="obf",
         .before=1) %>% 
  write_csv("dif_sequential_multi_easy_obf.csv")
# of_stage %>% View
